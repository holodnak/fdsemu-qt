...
struct {
    uint16_t filename[256];  //null terminated unicode string.  filename[0]: 0xFFFF=empty, 0x0000=multi-disk image (continued from previous)
    uint16_t lead_in;       //lead-in length (#bits), 0=default
     uint8_t reserved[254];   //set to 0
     uint8_t data[0xfe00];   //disk data, beginning with gap end mark (0x80) of first block
}
*/
...
bool block_decode(uint8_t *dst, uint8_t *src, int *inP, int *outP, int srcSize, int dstSize, int blockSize, char blockType) {
    ...
    if(calc_crc(dst+*outP,blockSize+2)) {
        ...
        printf("Bad CRC (%04X!=%04X) blockType %d, blockSize %d, in = %d, out = %d\n", crc1, crc2,blockType, blockSize, in / 8, out);
    }
...
//		  printf("dev_readDisk returned %d\n",result);
          bytesIn+=result;
...
//slot 1..n
bool FDS_writeFlash(char *filename, int slot,void *user, void(*callback)(void*,int)) {
    ...
    while(pos<filesize && inbuf[pos]==0x01) {
        ...
        if(fds_to_bin(outbuf+FLASHHEADERSIZE, inbuf+pos, SLOTSIZE-FLASHHEADERSIZE)) {
            ...
            outbuf[256]=DEFAULT_LEAD_IN & 0xff;
            outbuf[257]=DEFAULT_LEAD_IN / 256;
            if(side==0) {
...
//                ((uint16_t*)outbuf)[FILENAMELENGTH-1]=0;
                     strncpy((char*)outbuf, shortName, 256);
            }
            spi_writeFlash(outbuf, (slot+side)*SLOTSIZE, SLOTSIZE, pos, user, callback);
        }
        pos+=FDSSIZE;
        side++;
    }
...
int FDS_readHeader(int slot, uint8_t *buf)
{
    if(!spi_readFlash(slot * SLOTSIZE,buf,256))
        return(-1);
    return(0);
